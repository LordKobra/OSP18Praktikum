...
int incoming = 0;  //the client which is different for every child process
...
void directory(char buf[]){              //rueckgabe von actuellen "ort"(also einfach pwd)
    ...
    if(pipe(pipefd) != 0){
        ...
        if(write(incoming, "directory did not work", 22) < 0){
    ...
    if((pid=fork()) == 0){ //this is the child herleluyer
        ...
        if(write(incoming, "directory child did not work", 28) < 0){
    ...
    if(read(pipefd[0], buf, 512) <=0){                //ist falsch pls edit
        ...
        if(write(incoming, "directory reading did not work", 30) < 0){
...
int execute(char** args){
    ...
    if (strcmp(args[0],"cd") == 0){
        if(args[2] != NULL){
            ...
            if(write(incoming, "cd in execute did not work", 26) < 0){
...
void cd(char *path)
{
    ...
    if(write(incoming, "Error changing directory", 24) < 0){
...
void waitProcess(char ** args){
    ...
    if(write(incoming, "Error waiting on process: no process selected", 35) < 0){
    ...
    if(status == 0){
        ...
        if(write(incoming,"signal-handler configured wrong" , 32 )<0){
...
void get(char* filename, int clientid){
    ...
    if (fp) {
            while ((nread = fread(buf, 1, sizeof(buf), fp)) > 0)
                if (write(incoming, buf, sizeof(buf)) < 0)
...
void put(char* filename, int clientid ){
    ...
    fp = fopen(filename, "w");
    char buf[256];
    while((rval = read(incoming, buf, sizeof(buf))) > 0)
...
void backGr(char** args){      // for the & command
    ...
    if((pid = fork())==0){ // if child
        ...
        if (redir(1, "log.txt") < 0)
        {
            ...
            if(write(incoming, "Fehler beim Umleiten von stdout\n", 40)<0){
...
void parExec(char** args1, char ** args2){
    ...
    if(pipe(pipefd) != 0){
        ...
        if(write(incoming, "pipe did not work in parExec\n", 40)<0){
    ...
    if((pid[0] = fork()) == 0){ //first child
        ...
        if(write(incoming, "first fork in parExec did not work\n", 40) <0){
    ...
    if((pid[1] = fork()) ==0){ // second child
        ...
        if(write(incoming, "second fork in parExec did not work\n", 40) <0){
...
void normExec(char** args){
    ...
    if((pid = fork())==0){ //if child
        ...
        if(write(incoming, "fork in normExec did not work\n", 30) < 0){
